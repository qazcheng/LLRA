...
from ecc.eccrypt import encrypt,decrypt
import hashlib
...
class E_C:
    ...
    def mul(self,key):
        key = key % self.P
        a=mulp(self.A, self.B, self.P, self.G, key)
    ...
    def rsaenc(self,message,pk):
        pk=tuple(self.curve,pk)
        return encrypt(message,pk)
    def rsadec(self,message,pk,dk):
        dk=tuple(self.curve,dk)
        return decrypt(message,pk,dk)
